...
GO
--SqlScripter----[JobBloqueaUsuarioFT3web].sql
GO
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'[[Uncategorized (Local)]]' AND category_class=1)
   EXEC msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'[[Uncategorized (Local)]]'

DECLARE @jobId BINARY(16)
EXEC msdb.dbo.sp_add_job @job_name=N'JobBloqueaUsuarioFT3web',
		@enabled=1,
		@notify_level_eventlog=0,
		@notify_level_email=0,
		@notify_level_netsend=0,
...
GO
--SqlScripter----[PlanReindex&Statistics].sql
GO
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'[Database Maintenance]' AND category_class=1)
   EXEC msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'[Database Maintenance]'

DECLARE @jobId BINARY(16)
EXEC msdb.dbo.sp_add_job @job_name=N'PlanReindex&Statistics',
		@enabled=1,
		@notify_level_eventlog=2,
		@notify_level_email=0,
		@notify_level_netsend=0,
...
EXEC msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Subplan_1',
		...
		@os_run_priority=0,
		@subsystem=N'SSIS',
		@command=N'/Server "$(ESCAPE_NONE(SRVR))" /SQL "Maintenance Plans\PlanReindex&Statistics" /set "\Package\Subplan_1.Disable;false"',
		@database_name=N'',
		@flags=0
...
GO
--SqlScripter----[PlanCheckIntegrityDB.Subplan_1].sql
GO
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'[Database Maintenance]' AND category_class=1)
   EXEC msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'[Database Maintenance]'

DECLARE @jobId BINARY(16)
EXEC msdb.dbo.sp_add_job @job_name=N'PlanCheckIntegrityDB.Subplan_1',
		@enabled=1,
		@notify_level_eventlog=2,
		@notify_level_email=0,
		@notify_level_netsend=0,
...
GO
--SqlScripter----[PlanLimpiaHistorial&Log].sql
GO
IF NOT EXISTS (SELECT name FROM msdb.dbo.syscategories WHERE name=N'[Database Maintenance]' AND category_class=1)
   EXEC msdb.dbo.sp_add_category @class=N'JOB', @type=N'LOCAL', @name=N'[Database Maintenance]'

DECLARE @jobId BINARY(16)
EXEC msdb.dbo.sp_add_job @job_name=N'PlanLimpiaHistorial&Log',
		@enabled=1,
		@notify_level_eventlog=2,
		@notify_level_email=0,
		@notify_level_netsend=0,
...
EXEC msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Subplan_1',
		...
		@os_run_priority=0,
		@subsystem=N'SSIS',
		@command=N'/Server "$(ESCAPE_NONE(SRVR))" /SQL "Maintenance Plans\PlanLimpiaHistorial&Log" /set "\Package\Subplan_1.Disable;false"',
		@database_name=N'',
		@flags=0
