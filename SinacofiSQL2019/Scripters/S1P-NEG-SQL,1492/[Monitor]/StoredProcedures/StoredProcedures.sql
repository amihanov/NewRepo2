...
--        res.name    
 ...
 from master.sys.databases db  join msdb..backupset res 
    on db.recovery_model in (1,2) and db.name = res.database_name
...
BEGIN
   ...
   --select @asig as asig, @MB as mb
   --select @porcenusado
   --select @disco, @asig , @MB, @porcenusado
   IF @porcenusado > 80--5 and @porcenusado <= 95 
   begin
...
IF @ENVIAR_MENSAJE = 'Y' 
      BEGIN
                set @HTML_MENSAJE = N'<HTML><P>' + @MENSAJE + N'</P></HTML>'
                exec  msdb..sp_send_dbmail @profile_name = 'AJAX\SQLPROD01' 
...
GO
--SqlScripter----[dbo].[SP_EjecutaJobconEspera].sql
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER ON
GO
IF OBJECT_ID('[dbo].[SP_EjecutaJobconEspera]') IS NULL EXEC('CREATE PROCEDURE [dbo].[SP_EjecutaJobconEspera] AS PRINT ''TEMPORARY OBJECT, DELETE AFTER ALL OBJECTS ARE CREATED''')
GO
ALTER PROCEDURE [dbo].[SP_EjecutaJobconEspera]
(
@job_name SYSNAME,
...
GO
--SqlScripter----[dbo].[SP_MEM_MONITOR].sql
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER ON
GO
...
GO
--SqlScripter----[dbo].[SP_MonitorLoginFail].sql
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER ON
GO
IF OBJECT_ID('[dbo].[SP_MonitorLoginFail]') IS NULL EXEC('CREATE PROCEDURE [dbo].[SP_MonitorLoginFail] AS PRINT ''TEMPORARY OBJECT, DELETE AFTER ALL OBJECTS ARE CREATED''')
GO
ALTER PROCEDURE [dbo].[SP_MonitorLoginFail]
