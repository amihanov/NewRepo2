...
GO
--SqlScripter----[dbo].[sp_getdate_prueba].sql
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER ON
GO
IF OBJECT_ID('[dbo].[sp_getdate_prueba]') IS NULL EXEC('CREATE PROCEDURE [dbo].[sp_getdate_prueba] AS PRINT ''TEMPORARY OBJECT, DELETE AFTER ALL OBJECTS ARE CREATED''')
GO
ALTER PROCEDURE [dbo].[sp_getdate_prueba]
AS
select GETDATE()
GO
...
-- Author:    MSC -test
-- Create date: 24-06-2013
-- Description: Realiza la eliminaci≤n de datos en tablas historicas y de encabezado,  dado un criterio de meses a mantener en el sistema.
-- Author: HMF
...
BEGIN
  BEGIN TRY
    ...
    -- Eliminando historicos tablas Afiliados test
    --***************************************
    begin
      while (1=1)
      begin
 ...
 --*****************************************
  ...
  -- Procediendo a realizar limpieza tabla tblHistRecaudacionDetalle
    ...
    -- Eliminando historicos tablas Consolidaci≤n
  --*******************************************
    begin
  ...
  -- Procediendo a eliminar datos historicos tabla tblConsolidacionCarteraDetalle
    ...
    set @iCodError = 0;
    set @vcMsjError = 'Eliminaci≤n de datos historicos realizada exitosamente';
    return
  END TRY
  BEGIN CATCH
    IF @@TRANCOUNT > 0
...
GO
--SqlScripter----[dbo].[spListaPeriodoMora].sql
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER ON
GO
IF OBJECT_ID('[dbo].[spListaPeriodoMora]') IS NULL EXEC('CREATE PROCEDURE [dbo].[spListaPeriodoMora] AS PRINT ''TEMPORARY OBJECT, DELETE AFTER ALL OBJECTS ARE CREATED''')
GO
-- =============================================
-- Author:    GRN
-- Create date: 09-07-13  
...
-- =============================================
ALTER PROCEDURE [dbo].[spListaPeriodoMora]
AS
BEGIN
  Select 0 as iIdPeriodoMora,'Todos' as vcPeriodoMora
  union
  Select iIdPeriodoMora, vcPeriodoMora from tblPeriodoMora
END
GO
...
BEGIN
  ...
  BEGIN
    ...
    if @iCantidad>0 
      begin
        set @iCodError = 0; -- DEBE IR UN -1 CON EL CICLO COMPLETO
        set @vcMsjError = 'Ya hay un archivo en proceso no validado'; 
      end
    else
      begin   
...
END
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[spVerificaPeriodoCargaValidacion]') AND type in (N'P', N'PC'))
  print 'Procedimiento creado exitósamente'
else
print 'No se pudo crear el procedimiento almacenado.'
GO
