...
AS
  BEGIN
      DELETE FROM db_Application
      FROM   deleted
      WHERE  db_Application.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_Parameter
      FROM   deleted
      WHERE  db_Parameter.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_Op
      FROM   deleted
      WHERE  db_Op.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_Program
      FROM   deleted
      WHERE  db_Program.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_Gator
      FROM   deleted
      WHERE  db_Gator.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_Star
      FROM   deleted
      WHERE  db_Star.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_Field
      FROM   deleted
      WHERE  db_Field.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_Layout
      FROM   deleted
      WHERE  db_Layout.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_Index
      FROM   deleted
      WHERE  db_Index.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_Column
      FROM   deleted
      WHERE  db_Column.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_Table
      FROM   deleted
      WHERE  db_Table.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_DataType
      FROM   deleted
      WHERE  db_DataType.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_ServerType
      FROM   deleted
      WHERE  db_ServerType.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_InfoBlob
      FROM   deleted
      WHERE  db_InfoBlob.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_WKF
      FROM   deleted
      WHERE  db_WKF.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_WKFConnection
      FROM   deleted
      WHERE  db_WKFConnection.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_ProgramInstance
      FROM   deleted
      WHERE  db_ProgramInstance.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_ProgramType
      FROM   deleted
      WHERE  db_ProgramType.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_ProgramTypeMisc
      FROM   deleted
      WHERE  db_ProgramTypeMisc.dbrep_IID = deleted.dbrep_IID

      DELETE FROM db_PSAFile
      FROM   deleted
      WHERE  db_PSAFile.dbrep_IID = deleted.dbrep_IID
  END
GO
...
GO
--SqlScripter----[dbo].[HARENVIRONMENT_TEMPLATEID_TRIGGER].sql
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER ON
GO
IF OBJECT_ID('[dbo].[HARENVIRONMENT_TEMPLATEID_TRIGGER]') IS NOT NULL EXEC('DROP TRIGGER [dbo].[HARENVIRONMENT_TEMPLATEID_TRIGGER]')
GO
CREATE TRIGGER [dbo].[HARENVIRONMENT_TEMPLATEID_TRIGGER]
ON dbo.HARENVIRONMENT AFTER DELETE AS 								    UPDATE HARENVIRONMENT 									SET TEMPLATEID = NULL 									FROM HARENVIRONMENT 									JOIN DELETED 									ON HARENVIRONMENT.TEMPLATEID = Deleted.ENVOBJID
GO
...
GO
--SqlScripter----[dbo].[tng_tu_link].sql
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER ON
GO
IF OBJECT_ID('[dbo].[tng_tu_link]') IS NOT NULL EXEC('DROP TRIGGER [dbo].[tng_tu_link]')
GO
/****** Object:  Trigger dbo.tng_tu_link    Script Date: 12/1/2005 11:50:45 PM ******/


CREATE TRIGGER [dbo].[tng_tu_link]
ON tng_link for update as
set nocount on
...
declare @timestamp datetime      
  ...
  begin
    select 80002	
	BEGIN PRINT 'Server: Msg '+cast(80002 as varchar)+', Level 16, State 1, Line 0'+char(13)+'Do not allow to update class name or uuid or implied or parent_link of a link object, the transaction failed. '; RAISERROR (80002, 16, 1); END;
...
GO
--SqlScripter----[dbo].[tng_tu_managedobject].sql
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER OFF
GO
IF OBJECT_ID('[dbo].[tng_tu_managedobject]') IS NOT NULL EXEC('DROP TRIGGER [dbo].[tng_tu_managedobject]')
GO
CREATE TRIGGER [dbo].[tng_tu_managedobject]
ON tng_managedobject for update as
declare @uuid_bin UUID       
declare @uuid int
...
set nocount on
  ...
  if( update(severity) or update(status_no))
	...
	begin
	  select 80001	
	  BEGIN PRINT 'Server: Msg '+cast(80001 as varchar)+', Level 16, State 1, Line 0'+char(13)+'Update severity or status_no failed Since one of acknowledges is on(1). '; RAISERROR (80001, 16, 1); END;
	  rollback transaction
	  return
	end
  ...
  begin
    select 80002	
	BEGIN PRINT 'Server: Msg '+cast(80002 as varchar)+', Level 16, State 1, Line 0'+char(13)+'Do not allow to update class name or uuid of an object, the transaction failed. '; RAISERROR (80002, 16, 1); END;
	rollback transaction
	return
  end
