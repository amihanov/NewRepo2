...
AS select * from tblConsolidacionDetalle where nPeriodo=201408
GO

GO
--SqlScripter----[dbo].[vis_tblIndicadorResumen_jul_dic].sql
GO
SET ANSI_NULLS ON
SET QUOTED_IDENTIFIER ON
GO
IF OBJECT_ID('[dbo].[vis_tblIndicadorResumen_jul_dic]') IS NULL EXEC('CREATE VIEW [dbo].[vis_tblIndicadorResumen_jul_dic] AS /*TEMPORARY OBJECT, DELETE AFTER ALL OBJECTS ARE CREATED*/ SELECT 0 C1')
GO
ALTER VIEW [dbo].[vis_tblIndicadorResumen_jul_dic]
AS select * from tblIndicadorResumen where nPeriodo in( 201407,201412)
GO
