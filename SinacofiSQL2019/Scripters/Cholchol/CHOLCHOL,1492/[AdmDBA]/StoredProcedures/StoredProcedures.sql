...
SELECT [vcFechaRegistro]
  ...
  where vcFechaRegistro >= convert(varchar, getdate(), 112) and vcHoraRegistro >= replace(convert(varchar, dateadd(MINUTE,-1,getdate()),108),':','') 
...
SET @vcCorreo = '<div style="font-family:Verdana;font-size:18px">
...
SET @vcCorreo = N'<HTML><P>' + @vcCorreo +  N'</P></table></HTML><hr>'
 + '<p>'
 + ' <div style="font-family:Verdana;font-size:10px">'
 + '<FONT COLOR="green">Este mensaje ha sido generado automaticamente desde '+ @@servername + ' ' + (SELECT CONVERT(VARCHAR(26), GETDATE(), 109)) + '.'
 + '</p>'
...
*		@fechaInicio Fecha inicio de recepcion para filtrar resultados
*		@fechaFin Fecha final de recepcion para filtrar resultados
*
* Salida:
...
exec  msdb..sp_send_dbmail
	@profile_name = @vcProfileName,
    --@recipients = N'nhuenul@sinacofi.cl', 
    --@recipients = N'seginfo@sinacofi.cl', 
    --@copy_recipients =N'area_base_datos@sinacofi.cl',
