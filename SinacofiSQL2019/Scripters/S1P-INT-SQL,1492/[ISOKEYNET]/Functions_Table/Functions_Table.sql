...
GO
--SqlScripter----[dbo].[fn_usr_getDocsDistribPolicies_OLD].sql
GO
SET ANSI_NULLS OFF
SET QUOTED_IDENTIFIER ON
GO
IF OBJECT_ID('[dbo].[fn_usr_getDocsDistribPolicies_OLD]') IS NULL EXEC('CREATE FUNCTION [dbo].[fn_usr_getDocsDistribPolicies_OLD] () RETURNS @Ret TABLE (c1 int) AS BEGIN /*TEMPORARY OBJECT, DELETE AFTER ALL OBJECTS ARE CREATED*/ RETURN END')
GO
ALTER FUNCTION [dbo].[fn_usr_getDocsDistribPolicies_OLD]
(
		@IKCDU int
	)
...
DECLARE curPOSITIONGROUPS CURSOR FOR
	SELECT PATH_INT,TIPO_DISTRIBUCION, CODIGO, CODIGODISTRIBUIDOR, CODIGODOCUMENTO FROM DISTRIBUCION D
	JOIN  fn_usr_getDistributionGroups(@IKCDU) T ON D.CODIGOGRUPO=IKPOSITIONSGROUPSTREEID
/*
	JOIN  VISTA_POSITIONSINGROUPTREE T ON D.CODIGOGRUPO=T.IKPOSITIONSGROUPSTREEID
	WHERE 
		POSITIONID IN (SELECT CODIGOCARGO FROM CARGOSUSUARIOS WHERE CODIGOUSUARIO=@IKCDU)
		OR
		CODIGOUSUARIO IN (SELECT CODIGOGRUPO FROM WEBGROUPMEMBERS WHERE CODIGOUSUARIO=@IKCDU)
...
FETCH NEXT FROM curPOSITIONGROUPS INTO @GROUP_PATH, @TIPO_DISTRIBUCION,@CODDIS,@CODIGODISTRIBUIDOR,@DOCCODE
WHILE @@FETCH_STATUS = 0
BEGIN
	--CARGOS SIMPLES + MULTI SIMPLE DE GRUPOS DE DISTRIBUCION
	INSERT INTO @DISTRI_AUX 
	...
	,@TIPO_DISTRIBUCION
	FROM VISTA_ORGANIGRAMA
	WHERE (TIPO=1 OR TIPO=5) 
	
	AND CODIGO IN (
			SELECT CODIGOCARGO FROM CARGOSUSUARIOS WHERE CODIGOUSUARIO=@IKCDU)
...
--CARGOS MULTI DE GRUPOS DE DISTRIBUCION
	...
	'(' + O.DESCRIPCION + ') ' + LTRIM(RTRIM(REPLACE(U.USUARIO,'''',' '))) 
	...
	WHERE (TIPO=4) AND WEBGROUPMEMBERS.CODIGOUSUARIO=@IKCDU 
	AND O.CODIGO IN (
		(SELECT POSITIONID FROM IKPOSITIONSGROUPS PG
		JOIN IKPOSITIONSGROUPSTREE G ON G.IKPOSITIONSGROUPSTREEID=PG.IKPOSITIONSGROUPSTREEID
		WHERE G.PATH_INT LIKE @GROUP_PATH  OR G.PATH_INT LIKE @GROUP_PATH + ',%' OR (CONVERT(NVARCHAR(100),G.IKPOSITIONSGROUPSTREEID)=@GROUP_PATH)))
...
SELECT 
	...
	MAX(TIPO) ,
	TIPO_DISTRIBUCION ,
MAX(dbo.ik_org_ImageOfNode(TIPO)),
'Distribuir' as TIPO_DISTRIBUCIONDESC
